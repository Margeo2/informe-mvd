...
def group_by_program_type(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df_grouped = df.groupby('TIPO_PROGRAMA', observed=False).agg({'ID_INSCRIPCION': 'nunique'}).reset_index()
    df_grouped = df_grouped.rename(columns={'TIPO_PROGRAMA': 'PROGRAMA', 'ID_INSCRIPCION': 'INSCRIPCIONES'})
    total_row = pd.DataFrame({'PROGRAMA': ['TOTAL'], 'INSCRIPCIONES': [df_grouped['INSCRIPCIONES'].sum()]})
